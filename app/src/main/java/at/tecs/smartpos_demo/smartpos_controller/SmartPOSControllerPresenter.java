...
public class SmartPOSControllerPresenter implements SmartPOSControllerContract.Presenter {
    ...
    public void loadCards() {
        try {
            ...
            Gson gson = new Gson();
            Type listCardType = new TypeToken<List<MifareCard>>() {
            }.getType();
    ...
    public void open() {
        ...
        smartPOSController.openCardReader(10000, Command.CARD_TYPE.M0, new SmartPOSController.OpenCardReaderListener() {
            ...
            public void onDetected(CardControl cardControl, int cardType, byte[] UUID) {
                ...
                RFReturnCode returnCode = cardControl.close();
            ...
            public void onError(RFReturnCode rfReturnCode) {
                ...
                view.log("RFOpen -> onDetected - RFClose called!");
                RFReturnCode returnCode = smartPOSController.closeCardReader();
                view.log("RFOpen -> onDetected - RFClose - Return code: " + returnCode.name());
    ...
    public void performTest1() {
        ...
        smartPOSController.openCardReader(100000, Command.CARD_TYPE.M0, new SmartPOSController.OpenCardReaderListener() {
            @Override
            public void onDetected(CardControl cardControl, int cardType, byte[] UUID) {
                view.log("RFOpen -> onDetected!");
                view.log("RFOpen -> onDetected - UUID: " + ByteUtil.bytes2HexStr_2(UUID));
                ...
                RFReturnCode authCode = cardControl.authenticateM0(ByteUtil.hexStr2Bytes("5DE3D9C591CF4CA241E3E1482EC5AE04"));
                ...
                Pair<RFReturnCode, byte[]> returnCodes = cardControl.readBlock(0x00);
                ...
                view.log("RFOpen -> onDetected - RFReadBlock [0x01] called !");
                returnCodes = cardControl.readBlock(0x01);
                view.log("RFOpen -> onDetected - RFReadBlock [0x01] - Return code:" + returnCodes.first.name());
                view.log("RFOpen -> onDetected - RFReadBlock [0x01] - Data:" + ByteUtil.bytes2HexStr_2(returnCodes.second));
    ...
    public void readAll() {
        ...
        smartPOSController.openCardReader(30000, Command.CARD_TYPE.M0, new SmartPOSController.OpenCardReaderListener() {
            ...
            public void onDetected(CardControl cardControl, int ct, byte[] bytes) {
                ...
                if (key != null && !key.isEmpty()) {   //Performing authentication
                    ...
                    Pair<RFReturnCode, byte[]> response = cardControl.transmit(Utils.calcCrc(ByteUtil.hexStr2Bytes("1A00")));

                    if (response.first != RFReturnCode.SUCCESS || response.second.length != 11 || response.second[0] != (byte) 0xAF) {
                        view.log("Invalid tag key");

                        RFReturnCode close = cardControl.close();
                    ...
                    try {
                        finalResponse = cardControl.transmit(Utils.calcCrc(requestAB));
                ...
                try {
                    ...
                    smartPOSController.openPrinter(new SmartPOSController.OpenPrinterListener() {
                        ...
                        public void onOpened(PrinterControl printerControl) {
                            ...
                            if (printerControl.feedLine(10) == PrinterReturnCode.SUCCESS) {
                                view.log("Feed line - Success!");
                            } else {
                                view.log("Feed line - Failed!");
                            }

                            if (printerControl.close() == PrinterReturnCode.SUCCESS) {
            ...
            public void run() {
                smartPOSController.openPrinter(new SmartPOSController.OpenPrinterListener() {
                    ...
                    public void onOpened(PrinterControl printerControl) {
                        ...
                        for (int i = 0; i < 10; i++) {
                            printerControl.print(i + "----", PrinterPrintType.TEXT);
                            printerControl.feedLine(1);
                        }
                        printerControl.feedLine(8);
    ...
    public void close() {
        new Thread(new Runnable() {
            @Override
            public void run() {
                if (smartPOSController.isCardReaderConnected()) {
